    ...
    <form class="form form-horizontal" id="form-admin-add" action="<?php echo U('Admin/Shop/edit');?>" method="post" enctype="multipart/form-data">
        ...
        <div class="row cl">
            ...
            <div class="formControls col-xs-8 col-sm-9">
                ...
                <!-- 单条信息城市 -->
                <?php if(is_array($data)): foreach($data as $key=>$vosscs): ?><!-- 所有城市 -->
                    <?php if(is_array($reschengs)): foreach($reschengs as $key=>$vocs): ?><option value="<?php echo ($vocs["id"]); ?>" <?php if($vocs['id'] == $vosscs['depchengshi']): ?>selected="selected"<?php endif; ?> ><?php echo ($vocs["mingch"]); ?></option><?php endforeach; endif; endforeach; endif; ?>
                </select>
                </span>
            </div>
        </div>
